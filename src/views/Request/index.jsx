...
const getStatusIndicator = (status) => {
    switch (status) {
        case "success":
            return <Text color="white" size={15}>Ваша заявка успешно отправлено</Text>;
        case "error":
            return <Text color="white" size={15}>Произошла ошибка при отправке заявки</Text>;
        default:
            return null;
    }
};
                ...
                <Container m={isMobile ? "0 5px" : undefined}>
                    <Block color="red-500" p={isMobile ? 30 : 70} radius={12}>
                        <FlexBox gap={isMobile ? 30 : 80} direction={isMobile ? "column" : "row"}>
                            ...
                            <form style={{ width: isMobile ? "100%" : "40%" }} onSubmit={handleSubmit}>
                                ...
                                <Input
                                    ...
                                    onChange={setPhone}
                                />
                                <Input placeholder="Эл. почта" maxLength={50} type="email" value={email} onChange={setEmail} />
                                <FlexBox gap={20} justify="space-between" align={isMobile ? "flex-start" : "center"} direction={isMobile ? "column" : "row"}>
                                    <Button variant="primary" color="blue" type="submit" disabled={!accepted || status === "fetching"}>
                                        Отправить
                                    </Button>
